...
Remember [pieces](../getting-started/reusable-content-with-pieces.md)? Pieces are great! All we need is a way to accept form submissions to create them.
...
* We start by extending [apostrophe-pieces](../getting-started/reusable-content-with-pieces.md), creating a new type of piece to hold our form submissions. We use [schemas](../getting-started/schema-guide.md) to define our form fields, including the usual `title` field which we redeclare with a new label, `Subject`.
...
* In `setSubmitSchema`, we use the [subset method of apostrophe-schemas](../../modules/apostrophe-schemas/README.md#subset) to fetch a trimmed-down version of our schema with just the four fields we want to let website visitors submit. Otherwise, they would be asked to enter `tags`, `published` and potentially other fields that don't make sense for them. This way we can keep those things for our own administrative use.
* In `submit`, we implement a method that takes a form submission (in `req.body`, as is normal for Express) and asks the [convert method of apostrophe-schemas](../../modules/apostrophe-schemas/README.md#convert) to sanitize it as a form submission, populating a new `piece` object with clean data. Then that method inserts it into the database with the [insert method of apostrophe-pieces](../../modules/apostrophe-pieces/README.md#insert).
...
* We override the [`self.output` method of apostrophe-widgets](../../modules/apostrophe-widgets/README.md#output). It will still render `views/widget.html`, but we want to pass some extra data.
...
* Here we follow the [super pattern](../../glossary.md#super-pattern) to make sure we don't lose the data provided by the `apostrophe-widgets` version of `getCreateSingleton`.
...
* We define a widget manager for our `contact-form` widgets, with a custom widget player method. We've [seen this before in the custom widgets tutorial.](../getting-started/custom-widgets.md#adding-a-javascript-widget-player-on-the-browser-side)
