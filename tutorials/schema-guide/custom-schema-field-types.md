...
Apostrophe's [schemas](../getting-started/schema-guide.md) provide a simple and powerful way to add new properties to any doc type in Apostrophe, such as a piece or a page. It's a powerful feature. But sometimes you might want to add a field type that doesn't already exist.
...
In addition, we saw a call to `pushCreateSingleton`. This method creates an object to represent our module on the browser side. It'll look for one with a [moog type name](../../glossary.md#moog-type) that matches the module's name... and our `user.js` file will provide that:
...
We start by calling `apos.define` to create a [moog type](../../glossary.md#moog-type) on the browser side with the same name as the module. The `pushCreateSingleton` call earlier will take care of calling `apos.create` for us.
...
There are two excellent examples of custom field types already built as separate modules in Apostrophe: [apostrophe-attachments](../../modules/apostrophe-attachments/README.md) and [apostrophe-video-fields](../../modules/apostrophe-video-fields/README.md). You can [learn from the code on github](../../modules/apostrophe-video-fields/README.md). The video field type is the simpler of the two. There's no magic here: if you build a field type using a module in your project in exactly the same way, or publish it to npm, it will work just as well.