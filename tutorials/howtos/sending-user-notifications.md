...
Sending a notification in a [command-line task](/tutorials/advanced-development/04-apostrophe-cli/command-line-tasks.md) works just as shown above. However, if you wish to send one at the *end* of the task, you should `await` the `apos.notify` call. This ensures that the notification reaches the database before the task process exits.
