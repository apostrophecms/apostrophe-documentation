...
In 2.x, [every widget is powered by a module that extends `apostrophe-widgets`](/api/apostrophe-widgets/README.md). You can do that too. And all widgets support schemas "out of the box."
...
Just for clarity's sake, and for parallelism with the [apostrophe-images](/api/apostrophe-images/README.md) subclass of [apostrophe-pieces](/api/apostrophe-pieces/README.md), which it partners with.
...
The second problem has no "baked-in" solution yet in 2.x. We will certainly be examining the issue as we encounter projects that call for it. However, note that you can use [apostrophe-custom-pages](/api/apostrophe-custom-pages/README.md) to build your own solutions, especially with the new `dispatch` method.
...
Also check out the new `park` option for the [apostrophe-pages](/api/apostrophe-pages/README.md) module:
